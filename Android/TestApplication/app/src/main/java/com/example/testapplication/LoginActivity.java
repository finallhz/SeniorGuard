...
public class LoginActivity extends Activity {                 //登录界面活动  
    ...
    //下面是所有的static属性块，在进行完对应的方法后，直接使用static变量访问
    public String _uid;
    ...
    public void login() throws JSONException {                                              //登录按钮监听事件
        if (isUserNameAndPwdValid()) {
            String userName = mAccount.getText().toString().trim();    //获取当前输入的用户名和密码信息  
            String userPwd = mPwd.getText().toString().trim();
            SharedPreferences.Editor editor =login_sp.edit();
            String  res=mUserDataManager.findUserByNameAndPwd(userName, userPwd);
            if(res!=null&&res!=""){                                             //返回1说明用户名和密码均正确
                //保存用户名和密码  
                ...
                _gender = jsonObject.getString("gender");
                _age = jsonObject.getString("age");
                _role = jsonObject.getString("role");
                _email = jsonObject.getString("email");
                _phone = jsonObject.getString("phone");
                _address = jsonObject.getString("address");
                _birthdate = jsonObject.getString("birthdate");
                editor.putString("_uid", _uid);
                editor.putString("_gender", _gender);
                editor.putString("_age", _age);
                editor.putString("_role", _role);
                editor.putString("_email", _email);
                editor.putString("_phone", _phone);
                editor.putString("_address", _address);
                editor.putString("_birthdate", _birthdate);
    ...
    public void cancel() throws JSONException {
        ...
        if (isUserNameAndPwdValid()) {
            String userName = mAccount.getText().toString().trim();    //获取当前输入的用户名和密码信息  
            String userPwd = mPwd.getText().toString().trim();
            int result=mUserDataManager.deleteUserDatabyname(userName);
            Log.i("进入方法","进入方法");
            if(result==1){                                             //返回1说明用户名和密码均正确  
                ...
                mAccount.setText("");
                editor.remove("_uid");
                editor.remove("_gender");
                editor.remove("_age");
                editor.remove("_role");
                editor.remove("_email");
                editor.remove("_phone");
                editor.remove("_address");
                editor.remove("_birthdate");
                editor.commit();
            }else if(result==0){
